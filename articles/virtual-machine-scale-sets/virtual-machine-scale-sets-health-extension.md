...
> [!CAUTION]
> Application Health Extension expects to receive a consistent probe response at the configured port `tcp` or request path `http/https` in order to label a VM as *Healthy*. If no application is running on the VM, or you're unable to configure a probe response, your VM is going to show up as *Unhealthy* (Binary Health States) or *Unknown* (Rich Health States). [Application health samples](https://github.com/Azure-Samples/application-health-samples) hosts examples of applications emitting app health signals to a local endpoint.
