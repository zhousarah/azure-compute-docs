...
```
  ...
  > [!NOTE]
  > Prior to Service Fabric [version 10.1 CU4](service-fabric-versions.md#service-fabric-version-name-and-number-reference), it is necessary to provision to the compute a certificate which CSS will use for encrypting the store. It must be installed on all nodes and be declared using the fabric setting parameter "EncryptionCertificateThumbprint" under section "CentralSecretService" with it's SHA-1 thumbprint. We recommend that you use an encryption certificate that is different from your cluster certificate. From 10.1 CU4 onward, if this setting is not passed, CSS will default to using a cluster-managed certificate for encryption. An existing CSS deployment can be moved to the cluster-managed certificate by removing the thumbprint declaration in an upgrade.
