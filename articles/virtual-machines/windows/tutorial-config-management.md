...
With Azure [Change Tracking](/azure/automation/change-tracking/overview) and [Update Management](/azure/automation/update-management/overview), you can easily identify changes in your Windows virtual machines in Azure and manage operating system updates for those VMs.
...
You use a [Log Analytics](/azure/azure-monitor/logs/log-query-overview) workspace to collect data that is generated by features and services such as Update Management. The workspace provides a single location to review and analyze data from multiple sources.
...
* [Log Analytics](/azure/azure-monitor/logs/log-query-overview) workspace
* [Automation](/azure/automation/)
* A [Hybrid runbook worker](/azure/automation/automation-hybrid-runbook-worker), which is enabled on the VM
...
| **Groups to update** |For VMs hosted on Azure, define a query based on a combination of subscription, resource groups, locations, and tags. This query builds a dynamic group of Azure-hosted VMs to include in your deployment. </br></br>For VMs not hosted on Azure, select an existing saved search. With this search, you can select a group of these VMs to include in the deployment. </br></br> To learn more, see [Dynamic Groups](/azure/automation/update-management/configure-groups).|
| **Machines to update** |Select **Saved search**, **Imported group**, or **Machines**.<br/><br/>If you select **Machines**, you can choose individual machines from the drop-down list. The readiness of each machine is shown in the **UPDATE AGENT READINESS** column of the table.</br></br> To learn about the different methods of creating computer groups in Azure Monitor logs, see [Computer groups in Azure Monitor logs](/azure/azure-monitor/logs/computer-groups) |
|**Update classifications**|Choose all necessary update classifications.|
|**Include/exclude updates**|Select this option to open the **Include/Exclude** pane. Updates to be included and those to be excluded are on separate tabs. For more information on how inclusion is handled, see [Schedule an Update Deployment](/azure/automation/update-management/deploy-updates#schedule-an-update-deployment). |
...
For detailed information on Change Tracking, see [Troubleshoot changes on a VM](/azure/automation/troubleshoot/change-tracking).
