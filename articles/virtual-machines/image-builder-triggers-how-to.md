...
> [!NOTE]
> To prevent unnecessary build failures, automatic triggers will be deactivated if the image template build has failed a few times in a row (either manually or automatically triggered). You can still manually build the image template, and once a manual build succeeds, the automatic triggers will be reactivated.
