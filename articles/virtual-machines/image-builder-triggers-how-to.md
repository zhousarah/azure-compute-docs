...
> [!NOTE]
> To prevent unnecessary build failures, automatic triggers will be deactivated if the image template builds fail more than 50 times in a row (either manually or automatically triggered). You can still manually build the image template, and once a manual build succeeds, the automatic triggers will be reactivated.
