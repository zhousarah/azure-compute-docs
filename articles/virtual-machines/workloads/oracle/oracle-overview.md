...
You can effectively achieve high availability for your Oracle databases by using Azure NetApp Files [availability zone volume placement](/azure/azure-netapp-files/use-availability-zones) in combination with Oracle Data Guard for an HA architecture across zones. Alternatively, to eliminate the cost of Data Guard licenses and running VMs in the secondary zone, you can use the storage-based replication functionality of Azure NetApp Files. Azure NetApp Files volumes can be placed in the availability zone of your choice the same way and can then be replicated between zones within the region by using [cross-zone replication](/azure/azure-netapp-files/cross-zone-replication-introduction) (or to another region using [cross-region replication](/azure/azure-netapp-files/cross-region-replication-introduction)). 
