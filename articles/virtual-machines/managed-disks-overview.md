...
Managed disks offer two different kinds of encryption. The first is Server Side Encryption, which is performed by the storage service. The second one is Azure Disk Encryption, which you can enable on the OS and data disks for your VMs.
...
To learn the best practices for optimizing VM + Disk configurations so that you can achieve your desired performance, see [Design for high performance](premium-storage-performance.md).
